...
struct sr_if* sr_get_interface(struct sr_instance* sr, const char* name)
{
    ...
    while(if_walker)
    {
       if(strcmp(if_walker->name,name) == 0)
        { return if_walker; }
...
void sr_add_interface(struct sr_instance* sr, const char* name)
{
    ...
    if(sr->if_list == 0)
    {
        ...
        assert(sr->if_list);
        sr->if_list->next = 0;
        strncpy(sr->if_list->name,name,sr_IFACE_NAMELEN-1);
        sr->if_list->name[sr_IFACE_NAMELEN-1] = '\0'; /* Ensure null termination */
        sr->if_list->cost = 1; /* Default cost for RIP */
        return;
    }
    ...
    if_walker = if_walker->next;
    strncpy(if_walker->name,name,sr_IFACE_NAMELEN-1);
    if_walker->name[sr_IFACE_NAMELEN-1] = '\0'; /* Ensure null termination */
    if_walker->cost = 1; /* Default cost for RIP */
    if_walker->next = 0;
} /* -- sr_add_interface -- */ 
