...
int is_packet_valid(uint8_t *packet /* lent */,
  ...
  if (eHdr->ether_type == htons(ethertype_arp)) {
    ...
    if (ip_cksum(ipHdr, sizeof(sr_ip_hdr_t)) != ipHdr->ip_sum) {
      printf("***** -> IP packet checksum is incorrect.\n");
      return 0;
    }
    ...
    if (ipHdr->ip_p == ip_protocol_icmp) {
      ...
      if (icmp_cksum(icmpHdr, len - icmpOffset) != icmpHdr->icmp_sum) {
        printf("****** -> ICMP packet checksum is incorrect.\n");
        return 0;
      }
    } else if (ipHdr->ip_p == ip_protocol_udp) {
      unsigned int udp_off = sizeof(sr_ethernet_hdr_t) + sizeof(sr_ip_hdr_t);
      struct sr_udp_hdr* udp_hdr = (struct sr_udp_hdr*)(packet + udp_off);
      if (udp_hdr->checksum != 0) {
          struct sr_udp_hdr udp_tmp = *udp_hdr;
          udp_tmp.checksum = 0;
          const uint8_t* udp_payload = packet + udp_off + sizeof(struct sr_udp_hdr);
          uint16_t calc = udp_cksum(ipHdr, &udp_tmp, udp_payload);
          if (calc != udp_hdr->checksum) {
...
uint16_t udp_cksum(const struct sr_ip_hdr* ip_hdr, const struct sr_udp_hdr* udp_hdr, const uint8_t* payload) {
  ...
  struct sr_udp_pseudo_hdr pseudo;

  pseudo.src_ip   = ip_hdr->ip_src;
  pseudo.dst_ip   = ip_hdr->ip_dst;
  pseudo.zero     = 0;
  pseudo.protocol = ip_hdr->ip_p;
  pseudo.udp_len  = udp_hdr->length;
  ...
  memcpy(buf + sizeof(pseudo), udp_hdr, sizeof(struct sr_udp_hdr));
  memcpy(buf + sizeof(pseudo) + sizeof(struct sr_udp_hdr),
         payload,
         udp_len - sizeof(struct sr_udp_hdr));

  uint16_t result = cksum(buf, total_len);
  free(buf);
  return result;
}
